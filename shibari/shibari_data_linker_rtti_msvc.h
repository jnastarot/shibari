...
#define MSVC_RTTI_FIX(fix_rva, fixed_data) {\
    uint32_t fixed_data_ = main_module->get_image().is_x32_image() ? main_module->get_image().get_image_base() + fixed_data : fixed_data;\
    fixed_data_ += extended_module->get_module_position().get_address_offset();\
...
#define MSVC_RTTI_FIX64(fix_rva, fixed_data) {\
